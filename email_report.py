...
──────────────────────
  1. Cabecera DeskFlow + período
  2. Resumen global (kWh totales, coste total, precio/kWh)
  3. Por cada sala de profesores:
       • Indicadores: horas abiertas / consumo sala / consumo puestos / total sala
       • Tabla detalle por puesto (solo los que tuvieron actividad)
  4. Nota metodológica
"""
...
def _sala_section(sala: dict) -> str:
    ...
    # Filas de la tabla de puestos (solo los que tuvieron sesión)
    rows = ""
    ...
    <div style="margin:0 32px 32px;">
      ...
      <div style="display:flex;gap:10px;margin-bottom:16px;">
        ...
        <div style="flex:1;background:#FAECD6;border-radius:10px;padding:14px;text-align:center;">
          ...
          <div style="font-size:12px;color:#8A5E10;font-weight:700;margin-top:3px;">{sala['total_cost_eur']:.4f} €</div>
        </div>

      </div>

      <!-- Tabla detalle por puesto -->
      <div style="font-size:10px;color:#999;text-transform:uppercase;letter-spacing:.06em;font-weight:600;margin-bottom:8px;">
        Detalle por puesto &nbsp;·&nbsp; {WORKSTATION_DEVICE_DESC}
      </div>
      <table style="width:100%;border-collapse:collapse;font-size:12px;">
        <thead>
          <tr style="background:#F7F4EE;">
            <th style="padding:8px 14px;text-align:left;font-size:10px;color:#888;font-weight:600;text-transform:uppercase;letter-spacing:.05em;">Código</th>
            <th style="padding:8px 14px;text-align:left;font-size:10px;color:#888;font-weight:600;text-transform:uppercase;letter-spacing:.05em;">Puesto</th>
            <th style="padding:8px 14px;text-align:center;font-size:10px;color:#888;font-weight:600;text-transform:uppercase;letter-spacing:.05em;">Horas</th>
            <th style="padding:8px 14px;text-align:center;font-size:10px;color:#888;font-weight:600;text-transform:uppercase;letter-spacing:.05em;">kWh</th>
            <th style="padding:8px 14px;text-align:center;font-size:10px;color:#888;font-weight:600;text-transform:uppercase;letter-spacing:.05em;">Coste</th>
          </tr>
        </thead>
        <tbody>{rows}</tbody>
      </table>
...
def build_html_report(stats: dict) -> str:
    salas          = stats["salas"]
    sala_sections  = "".join(_sala_section(s) for s in salas)
    total_sp1      = next((s for s in salas if s["zone_code"] == "SP1"), None)
    total_sp2      = next((s for s in salas if s["zone_code"] == "SP2"), None)

    # Mini tabla resumen por sala en el encabezado
    resumen_filas = ""
    for s in salas:
        c = _c(s["zone_code"])
        resumen_filas += f"""
        <tr>
          <td style="padding:8px 12px;border-bottom:1px solid #eee;">
            <span style="display:inline-block;width:8px;height:8px;background:{c['accent']};border-radius:50%;margin-right:6px;"></span>
      ...
      <div style="max-width:700px;margin:32px auto;background:#fff;border-radius:12px;overflow:hidden;box-shadow:0 4px 24px rgba(0,0,0,.08);">
        ...
        <div style="background:#3D5A3E;padding:28px 32px;">
          <div style="display:flex;align-items:center;gap:12px;">
            ...
            <div>
              <div style="color:#fff;font-size:19px;font-weight:700;letter-spacing:-.3px;">DeskFlow</div>
        ...
        <div style="padding:24px 32px 16px;">
          ...
          <div style="font-size:23px;font-weight:700;color:#1A1814;letter-spacing:-.4px;">{stats['period'].capitalize()}</div>
        ...
        <div style="padding:0 32px 16px;">
          ...
          <!-- Mini tabla resumen por sala -->
          <table style="width:100%;border-collapse:collapse;font-size:12px;">
            <thead>
              <tr style="background:#F7F4EE;">
                <th style="padding:8px 12px;text-align:left;font-size:10px;color:#888;font-weight:600;text-transform:uppercase;">Sala</th>
                <th style="padding:8px 12px;text-align:center;font-size:10px;color:#888;font-weight:600;text-transform:uppercase;">Horas abiertas</th>
        ...
        <div style="margin:0 32px 24px;padding:14px 16px;background:#F7F4EE;border-radius:8px;font-size:11px;color:#777;line-height:1.7;">
          ...
          🏢 <strong>Dispositivos de sala</strong> ({SALA_DEVICE_DESC}) ×
             horas que la sala estuvo abierta (primera entrada → última salida del período).<br>
          💻 <strong>Puestos de trabajo</strong> ({WORKSTATION_DEVICE_DESC}) ×
             horas de sesión activa por puesto.<br>
          💡 Precio aplicado: <strong>{stats['price_per_kwh']:.2f} €/kWh</strong>
             (estimado; ajusta en <code>energy.py → PRICE_PER_KWH</code>).
        </div>
...
def send_energy_report(stats: dict, period_label: str = "semanal") -> bool:
    ...
    try:
        ...
        msg["Subject"] = (
            f"DeskFlow — Informe {period_label} de consumo energético · Salas de Profesores"
        )
        msg["From"] = GMAIL_USER
